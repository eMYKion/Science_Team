...
static double motorDiameter = 0.09;
...
int main(int argc, char **argv){
	...
	if(argc!=4){
		printf("Needed 3 command-line arguments!");
		return -1;
	}
	
	//sscanf(*(argv+1), "%lf", &accelConstant);
	//accelConstant = 0.040;
	sscanf(*(argv+1), "%lf", &startPosMeters);
	sscanf(*(argv+2), "%lf", &targetPosMeters);
	sscanf(*(argv+3), "%lf", &accelConstant );
	...
	//profile(1000, (int)( abs(startPosMeters)/metersPerStep ), 0);
	//delay(500);
	//profile(MIN_SPEED, 10, 1);
	profile(max_speed, (int)((targetPosMeters - startPosMeters)/metersPerStep), 1);
	//delay(500);
	//profile(1000, (int)((farFenceMeters - targetPosMeters)/metersPerStep), 0);
